        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
          ...
          <div>
            <label for="servicePlatformFee" class="block text-sm font-medium text-gray-700 mb-2">
              Deposit Fee (£) <span class="text-red-500">*</span>
            </label>
            <div class="relative">
              ...
              <input
                id="servicePlatformFee"
                v-model.number="formData.platform_fee"
                type="number"
                step="0.01"
                min="0"
                max="99999.99"
                ...
                :class="[
                  ...
                  errors.platform_fee ? 'border-red-500' : 'border-gray-300'
                ]"
                placeholder="0.00"
                @input="validatePlatformFee"
                @blur="validatePlatformFee"
              />
            </div>
            <p v-if="errors.platform_fee" class="mt-1 text-xs text-red-500">{{ errors.platform_fee }}</p>
...
const isFormValid = computed(() => {
  return (
    ...
    formData.value.platform_fee >= 0 &&
    !errors.value.name &&
    !errors.value.description &&
    !errors.value.platform_fee
  )
})
...
const validatePlatformFee = () => {
  const fee = formData.value.platform_fee
  if (fee < 0) {
    errors.value.platform_fee = 'Deposit Fee cannot be negative'
  } else if (fee > 99999.99) {
    errors.value.platform_fee = 'Deposit Fee cannot exceed £99,999.99'
  } else if (isNaN(fee)) {
    errors.value.platform_fee = 'Please enter a valid number'
  } else {
    errors.value.platform_fee = ''
  }
}
